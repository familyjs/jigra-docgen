...
function buildExamples(c: DocsConfigInterface) {
  ...
  o.push(`In \`jigra.config.ts\`:`);
  o.push(``);
  o.push(`\`\`\`ts`);
  o.push(
    `/// <reference types="@jigra/${slugify(
      c.name.replace(/([a-z])([A-Z])/g, '$1 $2'),
    )}" />`,
  );
  o.push(``);
  o.push(`import { JigraConfig } from '@jigra/cli';`);
  o.push(``);
  o.push(`const config: JigraConfig = {`);
  o.push(`  plugins: {`);
  o.push(`    ${c.name}: {`);
